...
def display_lineup(players):
    if players == None:
        ...
        header = "{:3}{:35}{:>6}{:>6}{:>6}{:>8}"
        line = "{:<3d}{:35}{:>6}{:>6}{:>6}{:8.3f}"
        print(header.format("", "Player", "POS", "AB", "H", "AVG"))
        print("-" * 64)
        i = 1
        for player in players:
            avg = get_batting_avg(player["at_bats"], player["hits"])
            print(line.format(i, player["name"], player["position"],
                              player["at_bats"], player["hits"], avg))
            i += 1
    print()    
